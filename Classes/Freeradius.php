...
class Freeradius
{
    ...
    public function setUser($package)
    {
        ...
        $passcheck = $db_ext->select('select * from radcheck where username = ? and attribute = ?', [$subscriber->username, 'Cleartext-Password']);
        if (!empty($passcheck)) {
            $db_ext->update(
            ...
            $db_ext->insert('insert into radcheck (username,attribute,op,value) values (?, ?, ?, ?)', [$subscriber->username, "Cleartext-Password", ":=", $subscriber->password]);
        }

        $profilecheck = $db_ext->select('select * from radcheck where username = ? and attribute = ?', [$subscriber->username, 'User-Profile']);
        if (!empty($profilecheck)) {
            ...
            $db_ext->insert('insert into radcheck (username,attribute,op,value) values (?, ?, ?, ?)', [$subscriber->username, "User-Profile", ":=", $profile]);
        }
    ...
    {
        $db_ext = DB::connection('freeradius');
        ...
        $checker = $db_ext->select('select * from radgroupcheck where groupname = ? and attribute = ?',
        ...
        if (!empty($checker)) {
            ...
            $db_ext->insert('insert into radgroupcheck (username,attribute,op,value) values (?, ?, ?, ?)',
                [$groupname, "Framed-Protocol", "==", 'PPP']);
        }

        $checker = $db_ext->select('select * from radgroupreply where groupname = ? and attribute = ?',
        ...
        if (!empty($checker)) {
            ...
            $db_ext->insert('insert into radgroupreply (username,attribute,op,value) values (?, ?, ?, ?)',
        ...
        $checker = $db_ext->select('select * from radusergroup where username = ? and groupname = ?',
            [$subscriber->username, $groupname]);
        if (!empty($checker)) {
            $db_ext->update(
                'update radusergroup set value = ? where username = ? and groupname = ?',
                [10, $speed_profile, $groupname]
            );
        } else {
            $db_ext->insert('insert into radusergroup (username,groupname,priority) values (?, ?, ?, ?)',
